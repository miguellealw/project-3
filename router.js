...
class RouterNode {
    ...
    handleMessage(messageJSON, rinfo) {
        ...
        if(isUpdate) {
            ...
            {
                const oldCost = this.routerTable[node];
                this.routerTable[node] = costToSender + cost;
                this.forwardingTable[node] = sender;
                console.log(`\nUpdated cost from ${ip_to_letter_map[this.ip]} (${this.ip}) to ${ip_to_letter_map[node]} (${node}) as ${this.routerTable[node]}.\nOld cost: ${oldCost}`);
    ...
    sendMessageAfterConvergence(sendTo) {
        if(this.isRunning) {
            const message = JSON.stringify({
                ...
                ip: this.ip,
                port: this.port,
                utaId: "Miguel: 1001837419 | Aryan: 1001910692",
